...
$a_regex = "(.*account name=)(.*password=.*)\/>$"
$c_regex = "(<controller host=.*port=.*)"
$account = "      <account name=""${env:APPDYNAMICS.AGENT.ACCOUNTNAME}"" password=""${env:APPDYNAMICS.AGENT.ACCOUNTACCESSKEY}"" />"
$controller = " <controller host=""${env:APPDYNAMICS_CONTROLLER_HOSTNAME}"" port=""${env:APPDYNAMICS.CONTROLLER.PORT}"" ssl=""${env:APPDYNAMICS.CONTROLLER.SSL.ENABLED}""> "
...
(Get-Content $configFile) | ForEach-Object { $_ -replace "$a_regex" , "$account" } | Set-Content $configFile

(Get-Content $configFile) | ForEach-Object { $_ -replace "$c_regex" , "$controller" } | Set-Content $configFile
